        ...
        <div className={s.currency_item_row}>
          ...
          <div className={s.currency_item_center}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart width={50} height={50} data={props.item.data}>
                ...
                <Area
                  type="monotone"
                  dataKey="value"
                  stroke={props.item?.status ? "#84BDB3" : "#E16767"}
                  fillOpacity={1}
                  fill={
                    props.item?.status ? "url(#colorUp)" : "url(#colorDown)"
                  }
                />
              </AreaChart>
            </ResponsiveContainer>
            <div className={s.currency_item_center_status}>
              ...
              <span className={props.item?.status ? s.green : s.red}>
                {props.item?.status ? `+` : `-`}
                {props.item.quote.toFixed(2)}%
              </span>
            </div>
          </div>
          <div className={s.currency_item_right}>
            <div className={s.currency_item_right_row}>
              <span className={s.currency_item_right_money}>
                {props.data.isToken ? "T " : "$ "}
                {props.data.cost.toFixed(2)}
              </span>
              <span className={s.currency_item_right_buy}>
                {" "}
                Can buy: {Math.floor(props.money / props.data.cost)}
              </span>
            </div>
          </div>
        </div>
        <div className={s.currency_item_buy}>
          {props.data.canBuy ? (
            ...
            <div className={s.canNotBuy_wrapper}>
              <div className={s.in}>
                <div
                  className={`${s.btn} ${s.with_right_border}`}
                  onClick={() => {
                    props.setOpen(true);
                    props.setPopUpData({
                      currency: true,
                      title: t("wallets.Replenishment"),
                      fOption:  t("wallets.Amount"),
                      SOption: "Payment service",
                      nameBtn: "Replenish",
                      SOptionPlace: "",
                      money: props.money,
                      cost: props.data.cost,
  ...
  return (
    <div className={s.wallets_currency_body}>
      {createData(data.quotes).map((item, iter) => (
        <CurrencyItem
          SOptionPlace={props.SOptionPlace}
          setPopUpData={props.setPopUpData}
          setOpen={props.setOpen}
          key={iter}
          data={data.currencies[iter]}
          money={props.profile.money}
          item={item}
        />
      ))}
    </div>
  ...
  return (
    <section className={s.wallets_wrapper}>
      <PopUp open={open} blur={10} close={setOpen}>
        <PopUpWallets
          money={popUpData.money}
          cost={popUpData.cost}
          invest={popUpData.invest}
          SOptionPlace={popUpData.SOptionPlace}
          currency={popUpData.currency}
          title={popUpData.title}
          close={setOpen}
          fOption={popUpData.fOption}
          SOption={popUpData.SOption}
          nameBtn={popUpData.nameBtn}
        />
      </PopUp>
        ...
        <div className={s.currency_item_table}>
          <div className={s.select_table}>
            <div
              className={`${s.select_item} ${
                selectTable === 0 && s.active_table
              }`}
              onClick={() => setSelectTable(0)}
            >
              <span>
                {t("wallets.Replenishment")}/{t("wallets.withdrawal")}
              </span>
            </div>
            <div
              className={`${s.select_item} ${
                selectTable === 1 && s.active_table
              }`}
          ...
          <div className={s.history_table}>
            <Table
              rows={3}
              columns={5}
              data={selectTable === 0 ? data.history.buy : data.history.buy}
            />
          </div>
        </div>
      </div>
